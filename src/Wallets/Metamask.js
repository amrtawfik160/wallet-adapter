...
export class MetamaskWalletAdapter {
  ...
  getChainId = async () => {
    const { chainId } = await this.getProvider().getNetwork()
    return chainId
  }
  ...
  sendTransaction = async ({ to, value, options = {} }) => {
    ...
    const transaction = {
      to: to,
      value: ethers.utils.parseEther(value.toString()), // Convert eth to wei
      ...options
    }
  ...
  onChainIdChanged = (callback) => {
    this.getProvider().on('network', (network) => {
      callback(network.chainId)
    })
  }
}
